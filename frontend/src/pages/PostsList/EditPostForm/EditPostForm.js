...
export default function EditPostForm() {
  UseWebsiteTitle("Zmiana treści posta");
  ...
  const editPost = async (e) => {
    ...
    formData.append("idPost", location.state.id);

    axios
      .post("http://localhost:3002/api/posts/editpost", formData)
      .then((res) => {
        if (res.data.message) {
  ...
  return (
    <div className={`${styles.main_div}`}>
      ...
      >
        ...
        <button className={`${styles.add_post_button}`} onClick={editPost}>
          Edytuj Post!
        </button>
